    ...
    <form onSubmit={submit} className="card p-4 md:p-6 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="number" min={0} value={guests} onChange={e=>setGuests(parseInt(e.target.value||'0'))} className="w-full rounded-xl border px-3 py-2"/>
        </div>
        <div>
          <label className="text-sm">Menú / Alergias</label>
          <input value={menu} onChange={e=>setMenu(e.target.value)} placeholder="Ej. Pollo, sin nueces" className="w-full rounded-xl border px-3 py-2"/>
        </div>
      </div>
      <div className="flex gap-2">
        {['Sí','No','Pendiente'].map(s=> (
          <button type="button" key={s} onClick={()=>setStatus(s as any)}
            className={`px-3 py-2 rounded-xl border ${status===s?'bg-rose-600 text-white border-rose-600':'bg-white'}`}>{s}</button>
        ))}
      </div>
