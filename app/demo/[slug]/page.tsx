    ...
    <main className="container py-10 space-y-8">
      <header className="text-center space-y-3">
        ...
        <div className="flex flex-col md:flex-row gap-2 items-center justify-center text-gray-600">
          ...
          <Link href={event.maps} className="flex items-center gap-2 underline"><MapPin className="h-4 w-4" /> {event.venue}</Link>
        </div>
      </header>
      ...
      <section className="grid md:grid-cols-3 gap-4">
        {event.gallery.map((g,i)=>(
          <div key={i} className="aspect-[9/16] rounded-2xl bg-gray-100 flex items-center justify-center">Foto {i+1}</div>
        ))}
      </section>

      <footer className="py-10 text-center text-sm text-gray-600">
        Comparte este enlace por WhatsApp o muestra el código QR (próximamente)
      </footer>
    </main>
  )
}
